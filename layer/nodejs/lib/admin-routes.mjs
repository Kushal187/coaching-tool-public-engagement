...
import { CLASSIFY_SYSTEM } from '../prompts/load.mjs';
...
router.get('/documents', async (req, res) => {
  try {
    ...
    if (req.query.content_type) {
      filters.push({ path: ['content_type'], operator: 'Equal', valueText: req.query.content_type });
    }
    if (req.query.doc_type) {
      filters.push({ path: ['doc_type'], operator: 'Equal', valueText: req.query.doc_type });
    }
    if (req.query.source_label) {
      filters.push({ path: ['source_label'], operator: 'Equal', valueText: req.query.source_label });
    }

    // Weaviate has no GROUP BY, so fetch all chunks then group server-side.
    // Paginate 2000 at a time to handle large collections.
    const BATCH = 2000;
    ...
    res.json({ documents, total, page: safePage, pageSize, totalPages });
  } catch (error) {
