...
Two-column layout. User left, Admin right.
Minimal shared-resource edges to keep arrows clean.
"""
...
output_path = os.path.join(output_dir, "coaching-tool-aws-architecture")
...
with Diagram(
    ...
    with Cluster("PRACTITIONER FLOW", graph_attr={
        ...
        # Bedrock ref for user side
        bedrock_u = Bedrock("Amazon Bedrock\n(All Lambdas → LLM)")

        # Data refs for user side
    ...
    with Cluster("Security & Observability", graph_attr={"bgcolor": "#F5F5F5", "fontsize": "16", "fontname": "Helvetica Bold", "fontcolor": "#333333"}):
        secrets = SecretsManager("Secrets Manager\n(Weaviate Key)")
        iam = IAM("IAM Roles\n(Least Privilege)")
        cw = Cloudwatch("CloudWatch\nLogs & Alarms")

    # ═══════════════════════════════════════════════
    #  PRACTITIONER CONNECTIONS (clean top-down)
    # ═══════════════════════════════════════════════
    ...
    # All user lambdas → Bedrock (single grouped arrow to label node)
    ...
    # ═══════════════════════════════════════════════
    #  ADMIN CONNECTIONS (clean top-down)
    # ═══════════════════════════════════════════════
