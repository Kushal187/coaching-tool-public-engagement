...
vi.mock('../../prompts/load.mjs', () => ({
  COACH_AGENT_PROMPT: 'Coach for {{question}} | Explanation: {{explanation}} | User so far: {{userResponse}}',
}));
...
describe('coachResponse', () => {
  ...
  it('returns coaching response for a valid question', async () => {
    ...
    mockRunAgentLoop.mockResolvedValueOnce('Great question! Let me help you articulate your goals.');
  ...
  it('detects resolution marker and marks question as addressed', async () => {
    ...
    mockRunAgentLoop.mockResolvedValueOnce(
      'You have clearly defined your goals. [[RESOLVED]]',
    );
  ...
  it('strips [[RESOLVED]] marker from the response message', async () => {
    const session = freshSession();
    mockRunAgentLoop.mockResolvedValueOnce('Well done! [[RESOLVED]]');
  ...
  it('updates question status to in-progress on first interaction', async () => {
    ...
    mockRunAgentLoop.mockResolvedValueOnce('Tell me more about your outreach strategy.');
  ...
  it('does not downgrade status from in-progress on subsequent turns', async () => {
    ...
    mockRunAgentLoop.mockResolvedValueOnce('Good, what else?');
  ...
  it('records coaching history on the question state', async () => {
    const session = freshSession();
    mockRunAgentLoop.mockResolvedValueOnce('Let me help.');
  ...
  it('sets activeQuestionId on the session', async () => {
    ...
    mockRunAgentLoop.mockResolvedValueOnce('Response');
  ...
  it('handles null agent loop response', async () => {
    const session = freshSession();
    mockRunAgentLoop.mockResolvedValueOnce(null);
  ...
  it('compiles userResponse from coaching history when resolved', async () => {
    ...
    mockRunAgentLoop.mockResolvedValueOnce('Excellent, you have clear goals. [[RESOLVED]]');
  ...
  it('builds system prompt with turn-count context after 3+ exchanges', async () => {
    ...
    mockRunAgentLoop.mockResolvedValueOnce('Final response. [[RESOLVED]]');
