...
export async function classifyIntent(userMessage, session) {
  ...
  try {
    ...
    // Validate action. Names match the corresponding prompt files under prompts/
    // (except 'general', which has no dedicated prompt file — it's inline in server.mjs).
    const validActions = ['coach-agent-open', 'coach-agent-continue', 'retrieval-agent', 'suggest-next', 'general'];
    ...
    // Fallback: if there's an active question, assume coach-agent-continue; otherwise general
    return {
      action: session.activeQuestionId ? 'coach-agent-continue' : 'general',
...
export async function routeMessage(userMessage, session) {
  ...
  switch (decision.action) {
    case 'coach-agent-open':
    case 'coach-agent-continue':
