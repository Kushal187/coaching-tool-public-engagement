...
import { buildSourceDocuments } from './agent-tools.mjs';

const DEFAULT_MODEL = process.env.CHATBOT_MODEL || 'gpt-5.1';
...
export async function runAgentLoop({
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    if (choice.finish_reason === 'stop' || !choice.message.tool_calls?.length) {
      return { text: choice.message.content, sources: extractSources(allToolResults) };
    }

    messages.push(choice.message);
    const results = await executeToolCalls(choice.message.tool_calls, toolImpls, messages);
    allToolResults.push(...results);
  }

  return { text: null, sources: extractSources(allToolResults) };
}

/**
 * Extract deduplicated source documents from raw tool call results.
 */
function extractSources(toolResults) {
  ...
  for (const r of toolResults) {
    try {
      ...
      const items = parsed.results || parsed.chunks || [];
      hits.push(...items);
    } catch { /* skip non-JSON or malformed */ }
  }
  return hits.length > 0 ? buildSourceDocuments(hits) : [];
}
