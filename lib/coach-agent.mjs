...
import {
  COACH_AGENT_OPEN_PROMPT,
  COACH_AGENT_CONTINUE_PROMPT,
 ...
 *
 * Picks between the "open" and "continue" prompts based on the orchestrator's
 * routing action:
 *   - 'coach-agent-open'     → opening turn on a newly-entered question
 *   - 'coach-agent-continue' → mid-conversation follow-up (default)
 */
function buildSystemPrompt(question, qState, session, action) {
  ...
  // Default to the continue prompt when no action is provided or it's anything
  // other than the explicit open action — continue is the safer fallback since
  // most coaching turns are mid-conversation.
  const basePrompt = action === 'coach-agent-open'
    ? COACH_AGENT_OPEN_PROMPT
    : COACH_AGENT_CONTINUE_PROMPT;
 ...
 * @param {string} [action] - The orchestrator routing action
 *   ('coach-agent-open' or 'coach-agent-continue'). Defaults to continue
 *   behavior when unspecified.
 * @returns {{ message: string, resolved: boolean, questionId: number }}
 */
export async function coachResponse(questionId, userMessage, session, action) {
  ...
  const systemPrompt = buildSystemPrompt(question, qState, session, action);
